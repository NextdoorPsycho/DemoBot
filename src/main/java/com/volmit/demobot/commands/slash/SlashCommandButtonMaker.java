...
public class SlashCommandButtonMaker extends SlashCommand {

    public SlashCommandButtonMaker() {
        this.name = "master-custom";
...
//        e.deferReply().queue();
        ...
        Button button = Button.danger("create-message", "[\u2800 \u2800 \u2800 \u2800 Click for Nothing! \u2800 \u2800 \u2800 \u2800]");

        e.getChannel().sendMessageEmbeds(embed.build()).setActionRow(button).queue();
        e.reply("Embed created!").setEphemeral(true).queue();
    }
}